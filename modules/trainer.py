...
class BaseTrainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if epoch % self.save_period == 0 or best or epoch==(self.epochs+1):
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        if save_best:
            best_path = os.path.join(self.checkpoint_dir, f'model_best_{str(epoch)}.pth')
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        train_loss = 0
        self.model.train()
        for batch_idx, (images_id, images, reports_ids, reports_masks, cls) in enumerate(self.train_dataloader):
            # batch个报告中最大有数据长度为reports_masks.shape[-1]，其他都用mask来表示，mask=0d代表后面没有单词
            images, reports_ids, reports_masks, cls = images.to(self.device), reports_ids.to(self.device), \
                                                 reports_masks.to(self.device), cls.to(self.device)
            # ouput:(B, maxReportLen, vabNum) 也就是报告中每个词汇量输出的概率
            output,clsLoss = self.model(images, reports_ids, cls, mode='train')
            loss = self.criterion(output, reports_ids, reports_masks, clsLoss)
        ...
        self.save_result(self.checkpoint_dir + 'train.txt', epoch, f"{str(epoch)}/{str(self.epochs)}", str(train_loss / len(self.train_dataloader)))

        self.logger.info('[{}/{}] Start to evaluate in the validation set.'.format(epoch, self.epochs))
        self.model.eval()
        with torch.no_grad():
            val_gts, val_res = [], []
            for batch_idx, (images_id, images, reports_ids, reports_masks, cls) in enumerate(self.val_dataloader):
                images, reports_ids, reports_masks,cls = images.to(self.device), reports_ids.to(
                    self.device), reports_masks.to(self.device), cls.to(self.device)

                output, _ = self.model(images, cls=cls, mode='sample')
            ...
            self.save_result(self.checkpoint_dir + 'val.txt', epoch, val_met)
        ...
        with torch.no_grad():
            test_gts, test_res = [], []
            for batch_idx, (images_id, images, reports_ids, reports_masks, cls) in enumerate(self.test_dataloader):
                images, reports_ids, reports_masks,cls = images.to(self.device), reports_ids.to(
                    self.device), reports_masks.to(self.device), cls.to(self.device)
                output, _ = self.model(images, cls=cls, mode='sample')
                reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
