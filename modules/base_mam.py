...
# from .mamba_module import CrossMamba, selfMamba, tgtSelfMamba
...
#     def __init__(self, d_model, dropout=0.1, h = 8, type="self"):
#         super(MambaAttention, self).__init__()
#         self.linears = clones(nn.Linear(d_model, d_model), 4)
#         self.attn = None
#         if type== "self":
#             self.attn_fn = selfMamba(d_model)
#         elif type=="tgt_self":
#             self.attn_fn = tgtSelfMamba(d_model)
#         else:
#             self.attn_fn = CrossMamba(int(d_model/h),  h)
#         self.dropout = nn.Dropout(p=dropout)
#         # self.keyLinear = nn.Linear(98, 59)
#         # self.valueLinear = nn.Linear(98, 59)

#     def forward(self, query, key, value, mask=None, layer_past=None):
#         # 不用mask因为对于图片，只要不对batch进行操作就不用担心提前知道的问题
#         # 不用mask因为对于图片，对于report他是[B, 57, 57, 512]的，对应57中情况下的单词情况
#         if mask is not None:
...
#         #     key = self.keyLinear(key.transpose(-1, -2)).transpose(-1, -2).contiguous()
#         #     value = self.valueLinear(value.transpose(-1, -2)).transpose(-1, -2).contiguous()
...
# class CorssMambaNet(nn.Module):
#     def __init__(self, encoder, decoder, src_embed, tgt_embed, cmn):
#         super(CorssMambaNet, self).__init__()
...
#             # ? 如何将tgt (B, reportmaxLen, 512) 与memory (B, 98, 512)融合
#             # 通过输入前的线性处理，或者直接cat
#             # ? 如何利用tgt_mask对数据进行遮挡
#             # 我可以tgt变成(B, reportmaxLen， reportmaxLen， d_model) 然后对reportmaxLen，进行遮挡，输入到模型中
...
# class BaseMAM(AttModel):

#     def make_model(self, tgt_vocab, cmn):
#         c = copy.deepcopy
#         # todo: 修改
#         selfAtten = MambaAttention(self.d_model, type="self")
#         corssattn = MambaAttention(self.d_model, h = self.num_heads, type="cross")
#         tgtSelfAttn = MambaAttention(self.d_model, type="tgt_self")
#         ff = PositionwiseFeedForward(self.d_model, self.d_ff, self.dropout)
#         position = PositionalEncoding(self.d_model, self.dropout)
#         # todo：修改
#         model = CorssMambaNet(
#             Encoder(EncoderLayer(self.d_model, c(selfAtten), c(ff), self.dropout), self.num_layers),
#             Decoder(DecoderLayer(self.d_model, c(tgtSelfAttn), c(corssattn), c(ff), self.dropout), self.num_layers),
...
#         super(BaseMAM, self).__init__(args, tokenizer)
#         self.args = args
...
#         out = self.model(att_feats, seq, att_masks, seq_mask, memory_matrix=self.memory_matrix) # Transformer  memory_matrix如何被融入的，以及seq_mask是如何使用的
